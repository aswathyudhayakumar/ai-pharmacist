 ...
 * invoked as a model tool-call in this route; the other four are declared
 * here for the documented contract and land as real tool-calls with S2/S3.
 */
const AGENT_TOOLS: Anthropic.Tool[] = [
  ...
  },
  {
    ...
    description:
      "Produce a pharmacist-facing reconciliation draft for a pending prescription. You are given the patient's context and a list of candidate findings already computed deterministically from the knowledge base (interactions, class duplications, generic-saving opportunities) — every fact in a candidate finding is already verified. Decide which findings are worth surfacing to the pharmacist, write a short plain-language explanation for each one you select using ONLY the facts already given for it, and draft the pharmacist's selections (which pending items to add, a coupon note, a delivery estimate). This tool only drafts — it can never execute, dispense, approve a substitution, or change a prescription (NFR-S2, enforced in lib/router.ts, not in the prompt).",
    input_schema: {
      type: "object",
      properties: {
        summary: { type: "string", description: "One-sentence overview of the draft for the pharmacist." },
        selectedFindingIds: {
          type: "array",
          items: { type: "string" },
        ...
        medsToAdd: {
          type: "array",
          items: { type: "string" },
          description: "Salts from the pending prescription to add to the order. Normally all of them unless a finding indicates otherwise.",
        },
        bestCoupon: { type: "string", description: "A short plain-language coupon/savings note, or omit if none applies." },
        earliestDelivery: { type: "string", description: "A short plain-language delivery estimate." },
      },
      required: ["summary", "selectedFindingIds", "explanations", "medsToAdd"],
    },
  },
];
...
const SYSTEM_PROMPT = `You are a clinical-safety drafting assistant embedded in a pharmacy app. A registered pharmacist reviews everything you produce before anything is dispensed — you draft, you never approve or dispatch.
...
> {
  ...
  try {
    ...
    if (err instanceof Anthropic.AuthenticationError) {
      ...
      return { ok: false, status: 500, error: "The pharmacist review service is not configured correctly." };
    }
    if (err instanceof Anthropic.RateLimitError) {
      console.error("Anthropic rate limit in /api/agent:", err.message);
      return { ok: false, status: 503, error: "The pharmacist review service is busy — please try again shortly." };
    }
    if (err instanceof Anthropic.APIError) {
      console.error("Anthropic API error in /api/agent:", err.status, err.message);
      return { ok: false, status: 502, error: "Could not reach the pharmacist review service." };
    }
    console.error("Unexpected error in /api/agent:", err);
    return { ok: false, status: 500, error: "Something went wrong preparing your prescription." };
  }

  if (message.stop_reason === "refusal") {
    console.error("Draft generation refused:", message.stop_details);
    return { ok: false, status: 502, error: "Could not prepare a draft — please request a pharmacist call instead." };
  }
  ...
  if (!toolUse) {
    ...
    return { ok: false, status: 502, error: "No draft was produced." };
  }

  const raw = toolUse.input as DraftToolInput;
  ...
  return NextResponse.json(
    {
      ...
      message: "This surface isn't built yet. S1_prescription is live; S2_search and S3_qa land in later phases.",
